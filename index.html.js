    ...
    const state = {
      supabase:null, session:null, user:null, configured:false, chatChannel:null, orderChannel:null, catalogChannel:null, refreshTimer:null,
      tab:"home", page:"app", products:loadJson("husky_products_cache", fallbackProducts), coupons:loadJson("husky_coupons_cache", fallbackCoupons),
    ...
    function chatCacheKey(){return state.user?.id?`husky_chat_cache_${state.user.id}`:"husky_chat_cache_guest"}
    function saveChatCache(){try{if(state.user) localStorage.setItem(chatCacheKey(),JSON.stringify((state.chatMessages||[]).filter(m=>m.id!=="welcome")));}catch(e){console.warn("Não salvou cache do chat",e)}}
    function loadChatCache(){try{return safeList(JSON.parse(localStorage.getItem(chatCacheKey())||"[]"),[])}catch(e){return []}}
    function saveCatalogCache(){try{localStorage.setItem("husky_products_cache",JSON.stringify(state.products));localStorage.setItem("husky_coupons_cache",JSON.stringify(state.coupons));}catch(e){}}
    ...
    async function loadProducts(){
      ...
      try{
        ...
        if(Array.isArray(data)){ state.products=data.map(p=>({...p, price:Number(p.price||0), filters:[p.name,p.description,p.tag].filter(Boolean).join(' ').toLowerCase().split(/\s+/)})); saveCatalogCache(); }
    ...
    async function loadCoupons(){
      ...
      try{
        ...
        if(error) throw error;
        if(Array.isArray(data)){ state.coupons=data.map(c=>({...c, discount_value:Number(c.discount_value||0), min_subtotal:Number(c.min_subtotal||0), min_items:Number(c.min_items||0)})); saveCatalogCache(); }
    ...
    async function loadChat(){
      if(!state.user){ state.chatMessages=[]; return; }
      const welcome={id:"welcome",from:'husky',text:'Oi! Eu sou o atendente virtual da Husky Confeiteiro. Como posso te ajudar hoje? 🐺💙',time:'agora',mascot:ASSETS.mascotWave};
      if(!state.supabase){ state.chatMessages=[welcome,...loadChatCache()]; return; }
      try{
        ...
        state.chatMessages=rows.length?rows:[welcome];
        saveChatCache();
      }catch(e){
        console.warn('Chat:',e.message);
        const cached=loadChatCache();
        state.chatMessages=cached.length?cached:[welcome];
      }
    }
    function subscribeRealtime(){
      ...
      // Catálogo e cupons atualizam mesmo sem login do cliente.
      state.catalogChannel=state.supabase.channel('cliente-catalogo-publico')
    ...
    function setTab(t){state.page='app';state.tab=t;render(); if(['coupons','menu','home'].includes(t)){setTimeout(()=>{loadCoupons().then(()=>render());loadProducts().then(()=>render());},50)} if(t==='chat'){setTimeout(()=>loadChat().then(()=>render()),50)} } window.setTab=setTab;
    ...
    async function sendChat(custom){
      ...
      const tempMsg={id:tempId,from:'user',text,time:now,pending:true};
      state.chatMessages=(state.chatMessages||[]).filter(m=>m.id!=='welcome').concat(tempMsg);
      saveChatCache();
      ...
      try{
        ...
        const {data,error}=await state.supabase
          .from('chat_messages')
          .insert({user_id:state.user.id,order_id:orderId,sender:'customer',message:text})
          .select('id,user_id,order_id,sender,message,created_at,read_at')
          .single();
        if(error) throw error;
        if(data){
          state.chatMessages=state.chatMessages.map(m=>m.id===tempId?{
            id:data.id,from:'user',text:data.message,time:new Date(data.created_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}),raw:data
          }:m);
          saveChatCache();
        }
        setTimeout(async()=>{await loadChat();render();},350);
      }catch(e){
        state.chatMessages=state.chatMessages.map(m=>m.id===tempId?{...m,error:true,text:m.text+'  (não enviado)'}:m);
        saveChatCache();
        alert('Não foi possível enviar para a gestão: '+e.message);
        render();
      }
    ...
    /* ===== PATCH VELOCIDADE + CHAT + CUPONS - 27/04 ===== */
    let __lastCatalogSnapshot = '';
    let __lastChatSnapshot = '';
    let __fastSyncRunning = false;
    function normalizeCouponRow(c){
      return {
        ...c,
        code: String(c.code||'').toUpperCase(),
        title: c.title || c.code || 'Cupom Husky',
        description: c.description || '',
        discount_type: c.discount_type || c.type || 'value',
        discount_value: Number(c.discount_value ?? c.value ?? 0),
        min_subtotal: Number(c.min_subtotal || 0),
        min_items: Number(c.min_items || 0),
        max_uses: Number(c.max_uses || 0),
        used_count: Number(c.used_count ?? c.used ?? 0),
        active: c.active !== false,
        free_delivery: !!c.free_delivery || c.discount_type === 'free_delivery' || c.type === 'free_delivery'
      };
    }
    function normalizeProductRow(p){
      return {
        ...
        visible_on_client: p.visible_on_client !== false,
        filters: [p.name,p.description,p.tag,p.category].filter(Boolean).join(' ').toLowerCase().split(/s+/)
      };
    }
    async function loadProducts(){
      if(!state.supabase) return;
      try{
        const {data,error}=await state.supabase
          ...
          .select('*')
          .eq('active',true)
          .eq('visible_on_client',true)
          .order('updated_at',{ascending:false,nullsFirst:false})
          .order('created_at',{ascending:false});
        if(error) throw error;
        if(Array.isArray(data)){
          const next=data.map(normalizeProductRow);
          const snap=JSON.stringify(next.map(p=>[p.id,p.name,p.price,p.stock,p.available,p.visible_on_client,p.updated_at]));
          if(snap!==__lastCatalogSnapshot || state.products.length!==next.length){
            state.products=next;
            __lastCatalogSnapshot=snap;
            saveCatalogCache();
          }
        }
      }catch(e){console.warn('Produtos fallback:',e.message)}
    ...
    async function loadCoupons(){
      ...
      try{
        ...
        if(error) throw error;
        if(Array.isArray(data)){
          const next=data.map(normalizeCouponRow);
          const snap=JSON.stringify(next.map(c=>[c.id,c.code,c.title,c.discount_type,c.discount_value,c.min_subtotal,c.min_items,c.active,c.updated_at,c.used_count]));
          if(snap!==__lastCatalogSnapshot+'coupons' || JSON.stringify(state.coupons.map(c=>[c.id,c.code,c.discount_value,c.active,c.updated_at]))!==JSON.stringify(next.map(c=>[c.id,c.code,c.discount_value,c.active,c.updated_at]))){
            state.coupons=next;
            saveCatalogCache();
          }
        }
    ...
    async function refreshCatalogFast(forceRender=false){
      if(__fastSyncRunning) return;
      __fastSyncRunning=true;
      try{
        const before=JSON.stringify({p:state.products.map(p=>[p.id,p.price,p.stock,p.available,p.updated_at]),c:state.coupons.map(c=>[c.id,c.code,c.discount_value,c.active,c.updated_at])});
        await Promise.allSettled([loadProducts(),loadCoupons()]);
        const after=JSON.stringify({p:state.products.map(p=>[p.id,p.price,p.stock,p.available,p.updated_at]),c:state.coupons.map(c=>[c.id,c.code,c.discount_value,c.active,c.updated_at])});
        if(forceRender || before!==after) render();
      }finally{__fastSyncRunning=false;}
    }
    async function loadChat(){
      if(!state.user){ state.chatMessages=[]; return; }
      const welcome={id:'welcome',from:'husky',text:'Oi! Eu sou o atendente virtual da Husky Confeiteiro. Como posso te ajudar hoje? 🐺💙',time:'agora',mascot:ASSETS.mascotWave};
      ...
      try{
        ...
        const next=rows.length?rows:[welcome];
        const snap=JSON.stringify(next.map(m=>[m.id,m.text,m.time,m.from]));
        if(snap!==__lastChatSnapshot){
          state.chatMessages=next;
          __lastChatSnapshot=snap;
          saveChatCache();
        }
      }catch(e){
        console.warn('Chat:',e.message);
    ...
    async function sendChat(custom){
      ...
      try{
        ...
        setTimeout(()=>loadChat().then(()=>render()),120);
      }catch(e){
        state.chatMessages=state.chatMessages.map(m=>m.id===tempId?{...m,error:true,text:m.text+'  (não enviado)'}:m);
        saveChatCache();
        render();
    ...
    function subscribeRealtime(){
      if(!state.supabase) return;
      [state.chatChannel,state.orderChannel,state.catalogChannel].filter(Boolean).forEach(ch=>{try{state.supabase.removeChannel(ch)}catch(e){}});
      state.chatChannel=state.orderChannel=state.catalogChannel=null;
      if(state.user){
        state.chatChannel=state.supabase.channel('cliente-chat-fast-'+state.user.id)
          .on('postgres_changes',{event:'INSERT',schema:'public',table:'chat_messages',filter:'user_id=eq.'+state.user.id},payload=>{
            const row=payload.new;
            if(!row || state.chatMessages.some(m=>m.id===row.id)) return;
            state.chatMessages=(state.chatMessages||[]).filter(m=>m.id!=='welcome').concat({id:row.id,from:row.sender==='customer'?'user':'husky',text:row.message,time:new Date(row.created_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}),mascot:row.sender==='customer'?null:ASSETS.mascotDefault,raw:row});
            saveChatCache();
            if(state.tab==='chat') render();
          })
          .on('postgres_changes',{event:'UPDATE',schema:'public',table:'chat_messages',filter:'user_id=eq.'+state.user.id},()=>{loadChat().then(()=>state.tab==='chat'&&render())})
          .subscribe(status=>console.log('cliente chat realtime',status));
        state.orderChannel=state.supabase.channel('cliente-orders-fast-'+state.user.id)
          .on('postgres_changes',{event:'*',schema:'public',table:'customer_orders',filter:'user_id=eq.'+state.user.id},()=>{loadOrders().then(()=>render())})
          .subscribe();
      }
      state.catalogChannel=state.supabase.channel('cliente-catalogo-fast')
        .on('postgres_changes',{event:'*',schema:'public',table:'products'},()=>refreshCatalogFast(true))
        .on('postgres_changes',{event:'*',schema:'public',table:'coupons'},()=>refreshCatalogFast(true))
        .on('postgres_changes',{event:'*',schema:'public',table:'store_settings'},()=>refreshCatalogFast(true))
        .subscribe(status=>console.log('cliente catálogo realtime',status));
    }
    /* ===== FIM PATCH VELOCIDADE ===== */
    ...
    async function logout(){
      try{ if(state.supabase) await state.supabase.auth.signOut({scope:'local'}); }catch(e){console.warn('logout',e.message)}
      try{
        Object.keys(localStorage).filter(k=>k.includes('husky_cliente_auth_v1')||k.includes('-auth-token')||k.startsWith('sb-')).forEach(k=>localStorage.removeItem(k));
        sessionStorage.clear();
      }catch(e){}
      state.user=null;state.session=null;state.orders=[];state.chatMessages=[];state.auth={...state.auth,mode:'login',step:'form',message:'Você saiu da conta.'};state.tab='home';state.page='app';
      if(state.chatChannel){state.supabase?.removeChannel(state.chatChannel);state.chatChannel=null;}
      if(state.orderChannel){state.supabase?.removeChannel(state.orderChannel);state.orderChannel=null;}
      render();
    ...
    /* ===== AJUSTE FINAL CLIENTE: CHAT SEM DELAY E CACHE SEGURO ===== */
    let __clienteChatSnapFinal = '';
    let __clienteChatPollingFinal = null;
    async function loadChatFinal(forceRender=false){
      if(!state.user){ state.chatMessages=[]; return; }
      const welcome={id:'welcome',from:'husky',text:'Oi! Eu sou o atendente virtual da Husky Confeiteiro. Como posso te ajudar hoje? 🐺💙',time:'agora',mascot:ASSETS.mascotWave};
      if(!state.supabase){ state.chatMessages=[welcome,...loadChatCache()]; return; }
      try{
        const {data,error}=await state.supabase
          ...
          .limit(500);
        if(error) throw error;
        const rows=(data||[]).map(m=>({id:m.id,from:m.sender==='customer'?'user':'husky',text:m.message,time:new Date(m.created_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}),mascot:m.sender==='customer'?null:ASSETS.mascotDefault,raw:m}));
        const pending=(state.chatMessages||[]).filter(m=>String(m.id).startsWith('tmp-'));
        const snap=JSON.stringify(rows.map(m=>[m.id,m.from,m.text,m.time]));
        if(snap!==__clienteChatSnapFinal || forceRender){
          __clienteChatSnapFinal=snap;
          state.chatMessages=[welcome,...rows,...pending.filter(p=>!rows.some(r=>r.text===p.text&&r.from===p.from))];
          saveChatCache();
          if(forceRender || state.tab==='chat') render();
          setTimeout(()=>{const box=document.querySelector('.chat-box'); if(box) box.scrollTop=box.scrollHeight;},30);
        }
      }catch(e){
        console.warn('loadChatFinal',e.message);
        const cached=loadChatCache();
        if(cached.length) state.chatMessages=[welcome,...cached];
      }
    }
    loadChat = loadChatFinal;

    async function sendChatFinal(custom){
      const input=$('chatText');
      const text=(custom||input?.value||'').trim();
      if(!text)return;
      if(!state.user){setTab('profile');return;}
      const tempId='tmp-'+Date.now();
      const tempMsg={id:tempId,from:'user',text,time:new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}),pending:true};
      state.chatMessages=(state.chatMessages||[]).filter(m=>m.id!=='welcome').concat(tempMsg);
      if(input) input.value='';
      saveChatCache();
      render();
      setTimeout(()=>{const box=document.querySelector('.chat-box'); if(box) box.scrollTop=box.scrollHeight;},30);
      try{
        if(!state.supabase) throw new Error('Supabase não configurado');
        const {data,error}=await state.supabase
          .from('chat_messages')
          .insert({user_id:state.user.id,order_id:null,sender:'customer',message:text})
          .select('id,user_id,order_id,sender,message,created_at,read_at')
          .single();
        if(error) throw error;
        state.chatMessages=state.chatMessages.map(m=>m.id===tempId?{id:data.id,from:'user',text:data.message,time:new Date(data.created_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}),raw:data}:m);
        saveChatCache();
        render();
        setTimeout(()=>loadChatFinal(true),120);
      }catch(e){
        state.chatMessages=state.chatMessages.map(m=>m.id===tempId?{...m,error:true,text:m.text+'  (não enviada)'}:m);
        saveChatCache();
        render();
        alert('Não foi possível enviar para a gestão: '+e.message);
      }
    }
    sendChat = sendChatFinal;
    window.sendChat = sendChatFinal;

    const __baseSubscribeRealtimeClienteFinal = subscribeRealtime;
    subscribeRealtime = function(){
      if(!state.supabase) return;
      try{ __baseSubscribeRealtimeClienteFinal(); }catch(e){console.warn('subscribe base',e.message)}
      if(__clienteChatPollingFinal) clearInterval(__clienteChatPollingFinal);
      __clienteChatPollingFinal=setInterval(()=>{
        if(state.user) loadChatFinal(state.tab==='chat');
      },700);
    };
    /* ===== FIM AJUSTE FINAL CLIENTE ===== */

    let __huskyRenderedOnce=false;
    const __oldRender=render;
    render=function(){
      __huskyRenderedOnce=true;
      window.__HUSKY_CLIENT_READY=true;
      return __oldRender.apply(this,arguments);
    };
    function withTimeout(promise, ms, label){
      return Promise.race([
        promise,
        new Promise(resolve=>setTimeout(()=>{
          console.warn((label||'operação')+' demorou demais; usando dados locais/fallback.');
          resolve(null);
        }, ms))
      ]);
    }
    setTimeout(()=>{
      ...
      if(root && !__huskyRenderedOnce && (!txt || txt.includes('Carregando Husky'))){
        showFatalError('O app demorou para iniciar','A conexão com o Supabase/CDN demorou demais. Limpe a sessão, recarregue ou publique novamente o pacote corrigido.');
      }
    },5000);
    (async function boot(){
      try{
        ...
        initSupabase();
        subscribeRealtime();
        // Renderiza imediatamente com os dados locais. Assim o cliente nunca fica preso na tela de carregamento.
        render();
        // Depois tenta atualizar do Supabase sem travar a tela.
        await Promise.allSettled([
          withTimeout(loadProducts(), 4500, 'produtos'),
          withTimeout(loadCoupons(), 4500, 'cupons')
        ]);
        render();
        if(state.refreshTimer) clearInterval(state.refreshTimer);
        state.refreshTimer=setInterval(async()=>{
          try{ await Promise.allSettled([loadCoupons(),loadProducts(), state.user?loadChat():Promise.resolve()]); if(state.tab==='chat'||state.tab==='coupons'||state.tab==='menu'||state.tab==='home') render(); }catch(e){console.warn('refresh público',e.message)}
        },2000);
