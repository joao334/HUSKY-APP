...
function setupGestaoRealtime(){
  if(!HAS_SUPABASE||!state.user) return;
  [state.chatChannel,state.ordersChannel,state.productsChannel].filter(Boolean).forEach(ch=>db.removeChannel(ch));
  state.chatChannel=db.channel('gestao-chat').on('postgres_changes',{event:'*',schema:'public',table:'chat_messages'},async()=>{await loadSupabaseData();render();}).subscribe();
  state.ordersChannel=db.channel('gestao-orders').on('postgres_changes',{event:'*',schema:'public',table:'customer_orders'},async()=>{await loadSupabaseData();render();}).subscribe();
  state.productsChannel=db.channel('gestao-products').on('postgres_changes',{event:'*',schema:'public',table:'products'},async()=>{await loadSupabaseData();render();}).subscribe();
}
async function upsertSupabase(table,payload){
  if(!HAS_SUPABASE||!state.user) return;
  try{const {error}=await db.from(table).upsert(payload); if(error) throw error;}catch(e){toast('Erro Supabase: '+e.message)}
}
...
function setRoute(route, sub="overview"){state.route=route;state.subroute=sub;state.selectedOrder=null;render()}
...
async function sendGestaoChat(){
  ...
  const local={id:String(Date.now()),user_id:thread.user_id,order_id:thread.messages[thread.messages.length-1]?.order_id||null,order_number:thread.order_number,customer:thread.customer,sender:'store',message:msg,created_at:new Date().toISOString(),read:true};
  state.data.chats.push(local); if(input)input.value=''; saveAll(); render();
  if(HAS_SUPABASE){
    if(!thread.user_id){toast('Conversa sem user_id. Não foi possível enviar ao cliente no Supabase.');return;}
    const {error}=await db.from('chat_messages').insert({user_id:thread.user_id,order_id:local.order_id,sender:'store',message:msg});
    if(error){toast('Erro ao enviar no Supabase: '+error.message);return;}
    await loadSupabaseData(); render();
  }
}
...
async function manualChatRefresh(){if(HAS_SUPABASE)await loadSupabaseData();render();toast('Chat atualizado')}
...
function genericOpen(key,id){const map={categories:categoryFields(),addons:addonFields(),coupons:couponFields(),expenses:expenseFields(),suppliers:supplierFields(),banners:bannerFields(),issues:issueFields(),refunds:refundFields()};const fields=map[key];const arr=state.data[key];const item=id?arr.find(x=>x.id===id):{id:String(Date.now())};document.getElementById('root').insertAdjacentHTML('beforeend',`<div class="modal"><div class="modal-box"><div class="modal-head"><h3>${id?'Editar':'Criar'}</h3><button class="btn light" onclick="render()">Fechar</button></div><div class="modal-body"><div class="form-grid">${fields.map(f=>`<div class="field ${f.wide?'wide':''}"><label>${f.label}</label>${f.type==='textarea'?`<textarea id="gf_${f.name}">${item[f.name]||''}</textarea>`:f.type==='bool'?`<select id="gf_${f.name}"><option value="true" ${item[f.name]?'selected':''}>Sim</option><option value="false" ${!item[f.name]?'selected':''}>Não</option></select>`:`<input id="gf_${f.name}" value="${item[f.name]??''}">`}</div>`).join('')}</div><button class="btn blue" onclick="genericSave('${key}','${item.id}',${!id})">Salvar</button></div></div></div>`)}
function genericSave(key,id,isNew){const map={categories:categoryFields(),addons:addonFields(),coupons:couponFields(),expenses:expenseFields(),suppliers:supplierFields(),banners:bannerFields(),issues:issueFields(),refunds:refundFields()};const fields=map[key];const arr=state.data[key];const item=isNew?{id}:arr.find(x=>x.id===id);fields.forEach(f=>{let val=document.getElementById('gf_'+f.name).value;if(f.type==='number'||f.type==='money')val=Number(val||0);if(f.type==='bool')val=val==='true';item[f.name]=val});if(isNew)arr.unshift(item);audit(`${key}_save`,id);saveAll();syncGenericToSupabase(key,item);render()}
function syncGenericToSupabase(key,item){if(!HAS_SUPABASE)return;const tableMap={categories:'product_categories',addons:'product_addons',coupons:'coupons',expenses:'expenses',suppliers:'suppliers',banners:'banners',issues:'internal_tickets',refunds:'refunds'};const table=tableMap[key];if(!table)return;let payload={id:item.id};if(key==='categories')payload={id:item.id,name:item.name,sort_order:Number(item.sort||0),active:item.active,schedule:{label:item.schedule||''}};if(key==='coupons')payload={id:item.id,code:item.code,title:item.title,discount_type:item.type,discount_value:Number(item.value||0),min_subtotal:Number(item.min_subtotal||0),min_items:Number(item.min_items||0),max_uses:Number(item.max_uses||0),active:item.active};if(key==='addons')payload={id:item.id,group_name:item.group,name:item.name,price:Number(item.price||0),stock:Number(item.stock||0),required:item.required,active:item.active};if(key==='banners')payload={id:item.id,title:item.title,image_url:item.image,link_type:item.link_type,link_value:item.link_value,sort_order:Number(item.sort||0),active:item.active};if(key==='expenses')payload={id:item.id,name:item.name,category:item.category,value:Number(item.value||0),date:item.date,payment_method:item.payment_method,quantity:Number(item.quantity||0),supplier:item.supplier,note:item.note};if(key==='suppliers')payload={id:item.id,name:item.name,phone:item.phone,email:item.email,product:item.product,avg_value:Number(item.avg_value||0),active:item.active};if(key==='issues')payload={id:item.id,order_number:item.order_number,customer:item.customer,type:item.type,priority:item.priority,responsible:item.responsible,status:item.status,note:item.note};if(key==='refunds')payload={id:item.id,order_number:item.order_number,customer:item.customer,value:Number(item.value||0),type:item.type,status:item.status,reason:item.reason,approved_by:item.approved_by};upsertSupabase(table,payload)}
