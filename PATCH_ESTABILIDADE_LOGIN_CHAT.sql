-- PATCH DE ESTABILIDADE: LOGIN, PERFIL E CHAT CLIENTE <-> GESTÃO
-- Rode este arquivo no Supabase SQL Editor depois do SQL principal.
...
select u.id, u.email, coalesce(u.raw_user_meta_data->>'name', split_part(u.email,'@',1)), coalesce(u.raw_user_meta_data->>'phone',''), coalesce(u.raw_user_meta_data->>'address',''), 'active'
from auth.users u
...
drop policy if exists "chat_update_own_or_admin" on public.chat_messages;
create policy "chat_update_own_or_admin" on public.chat_messages for update using (auth.uid() = user_id or public.is_admin()) with check (auth.uid() = user_id or public.is_admin());
